// Ping horário ao motor de IA da Avó Maria. Se cair, avisa por email — e avisa
...
export default async () => {
  ...
  try { store = getStore("avo-maria-health"); antes = (await store.get("estado")) || "up"; } catch { /* sem blobs: avisa na mesma na 1ª deteção */ }

  if (estado !== antes) {
    const assunto = up ? "✅ A Avó Maria recuperou — externatosantamariadebelem.com" : "🔴 A Avó Maria em baixo — motor de IA (Externato)";
    const texto = up
      ? "O motor de IA da Avó Maria voltou a responder ao ping horário. Tudo normal."
      : "O motor de IA da Avó Maria NÃO respondeu ao ping horário. O chat inteligente está em baixo — verifica a GEMINI_API_KEY no Netlify e a conta Google AI Studio.";
    try {
